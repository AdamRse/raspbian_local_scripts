...
run=true
t_alert=false;
mysql -D "raspi_general" -s -N  -e "UPDATE opt SET valeur = '$pid' WHERE nom_opt = 'monitoring_run'"
echo "Lancement du script de monitoring"

while $run;
do
  #récupération des options et commandes dans la bdd
  read -ra tabOpt -d $'\0' <<< $(mysql -D "raspi_general" -s -N  -e "SELECT valeur FROM opt WHERE nom_opt = 'temperature_warning_alert_trigger_limit' OR nom_opt = 'temperature_warning_alert_trigger' OR nom_opt = 'monitoring_ram_alert_limit' OR nom_opt = 'monitoring_ram_alert'")
  ...
  mysql -D "raspi_general" -s -N  -e "INSERT INTO ram (utilisee, partagee, buff_cache) VALUES ('${tabRam[8]}', '${tabRam[10]}', '${tabRam[11]}')"
  ...
  bddHorloge=$(mysql -D "raspi_general" -s -N  -e "SELECT valeur FROM opt WHERE nom_opt = 'monitoring_frequence'")
  if (( $bddHorloge > 0 ));
  then
    horloge=$bddHorloge
  fi
  ...
  if [ $(mysql -D "raspi_general" -s -N  -e "SELECT valeur FROM opt WHERE nom_opt = 'monitoring_run'") != $pid ]
  then
    run=false
  fi
done
echo "commande de sortie de boucle"
mysql -D "raspi_general" -s -N  -e "UPDATE opt SET valeur = '0'  WHERE nom_opt = 'monitoring_run'"
